...
3. **Initialize dependencies**
   ```powershell
   # Windows (recommended: use batch file to handle encoding automatically)
   .\scripts\setup.bat
   # Or use PowerShell directly (if encoding errors occur, run chcp 65001 first)
   ...
   > **Windows Encoding Issues**: If you encounter "terminator expected" or Chinese character encoding errors, use `.\scripts\setup.bat`, or run `chcp 65001` first to set the code page to UTF-8.
4. **Run**
   ```bash
